/* 
 * Solution to H2S04 molecule creation problem - broken, removed barrier that only lets one
 * molecule be created at a time (though still only rare race condition causes multiple molecules
 * to be created at once, given random thread start order, fails <10% of the time in creating 2 molecules.
...
void* sulfur(void* args) {
  ...
  // wait until both hydrogens exit
  for (i=0; i<2; i++) {
    sem_wait(sulfurwait_sem);
  }
  ...
  // wait until all 4 oxygens exit
  for (i=0; i<4; i++) {
    sem_wait(sulfurwait_sem);
  }
  
  //printf("sulfer awake after 4 oxygens exit\n");
  //fflush(stdout);
